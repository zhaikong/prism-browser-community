...
$PatchContract = Get-PrismKernelPatch $KernelLock "032-webgpu-template-identity.patch"
...
$PatchPath = Join-Path $PSScriptRoot "patches\032-webgpu-template-identity.patch"
$SourceFile = Join-Path $SourcePath "third_party\blink\renderer\modules\webgpu\gpu_adapter.cc"
$PredecessorFile = Join-Path $SourcePath "content\browser\speech\tts_win.cc"
$SourceMarkers = @(
    "ApplyPrismWebGpuTemplateIdentity",
    "PrismWindowsWebGpuArchitecture",
    "blackwell"
)

if (-not (Test-Path $PatchPath)) {
    throw "Prism WebGPU template identity patch is missing: $PatchPath"
}
$patchHash = (Get-FileHash -Algorithm SHA256 -Path $PatchPath).Hash.ToLowerInvariant()
if ($patchHash -ne $ExpectedPatchHash) {
    throw "Unexpected Prism WebGPU template identity patch hash: $patchHash"
}
...
if ($activeNinja.Count -gt 0) {
    throw "The Chromium build is still running. Wait for it to finish before applying patch 032."
}
if (-not (Test-Path $PredecessorFile) -or
    -not (Select-String -Path $PredecessorFile -Pattern "PrismTtsMtaWorker" -SimpleMatch -Quiet)) {
    throw "Patch 031 is missing. Apply the v17 TTS runtime patch before patch 032."
}
...
Copy-Item -Path $PatchPath -Destination (Join-Path $fingerprintPatchDirectory "032-webgpu-template-identity.patch") -Force
...
$appliedMarkerCount = @($SourceMarkers | Where-Object {
    Select-String -Path $SourceFile -Pattern $_ -SimpleMatch -Quiet
}).Count
if ($appliedMarkerCount -ne 0 -and $appliedMarkerCount -ne $SourceMarkers.Count) {
    throw "The WebGPU template identity patch is only partially applied. Restore a clean v17 source tree before continuing."
}
if ($appliedMarkerCount -eq 0) {
    & git.exe -C $RepositoryPath apply --check --unsafe-paths --directory=build/src $PatchPath
    if ($LASTEXITCODE -ne 0) {
        throw "The WebGPU template identity patch does not apply cleanly to the current Chromium source."
    }
    & git.exe -C $RepositoryPath apply --unsafe-paths --directory=build/src $PatchPath
    if ($LASTEXITCODE -ne 0) {
        throw "Could not apply the WebGPU template identity patch."
    }
}
...
$buildLock | Add-Member -NotePropertyName prismWebGpuTemplateIdentityPatchSha256 -NotePropertyValue $patchHash -Force
...
Write-Host ""
Write-Host "Prism WebGPU template identity patch is ready."
