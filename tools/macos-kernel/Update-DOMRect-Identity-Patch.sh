...
[[ -f "$patch_dir/024-domrect-seed-mixing.patch" ]] || patch_dir="$script_dir/../kernel-patches"
patch_path="$patch_dir/024-domrect-seed-mixing.patch"
...
[[ -f "$patch_path" && -d "$repository_path/.git" && -d "$fingerprint_path/.git" \
  && -f "$document_path" && -f "$build_lock_path" ]] \
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null; then
  echo "Chromium is still compiling. Wait for Ninja to finish before applying DOMRect seed mixing." >&2
  exit 1
fi
...
if actual != patch["sha256"]:
    raise SystemExit("DOMRect seed mixing patch hash mismatch")
...
grep -Fq "PrismRenderIdentityNoise" "$document_path" \
  || { echo "Coherent render identity v3 is missing. Apply patch 023 before patch 024." >&2; exit 1; }
mkdir -p "$fingerprint_path/patches/extra/fingerprint"
cp "$patch_path" "$fingerprint_path/patches/extra/fingerprint/024-domrect-seed-mixing.patch"
...
markers=("kNoiseMask = 0xffffu" "state *= 0x7feb352du" "state *= 0x846ca68bu")
marker_count=0
for marker in "${markers[@]}"; do
  grep -Fq "$marker" "$document_path" && ((marker_count += 1)) || true
done
if (( marker_count != 0 && marker_count != ${#markers[@]} )); then
  echo "DOMRect seed mixing is only partially applied." >&2
  exit 1
fi
...
lock["prismDomRectSeedMixingPatchSha256"] = next(
    item["sha256"] for item in contract["patches"]
    if item["file"] == "024-domrect-seed-mixing.patch")
lock["patches"] = [
...
echo "Prism DOMRect seed mixing patch is ready."
