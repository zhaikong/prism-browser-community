...
patch_path="$script_dir/patches/032-webgpu-template-identity.patch"
[[ -f "$patch_path" ]] || patch_path="$script_dir/../kernel-patches/032-webgpu-template-identity.patch"
source_file="$source_path/third_party/blink/renderer/modules/webgpu/gpu_adapter.cc"

IFS=$'\t' read -r expected_platform_commit expected_fingerprint_commit expected_patch_hash < <(python3 - "$lock_path" <<'PY'
import json, sys
lock = json.load(open(sys.argv[1]))
patch = next(item for item in lock["patches"] if item["file"] == "032-webgpu-template-identity.patch")
print("\t".join((
...
[[ -f "$patch_path" ]] || { echo "WebGPU template identity patch is missing: $patch_path" >&2; exit 1; }
[[ -f "$source_file" ]] || { echo "Prepared Chromium source is incomplete: $source_file" >&2; exit 1; }
[[ "$(shasum -a 256 "$patch_path" | awk '{print $1}')" == "$expected_patch_hash" ]] \
  || { echo "Unexpected WebGPU template identity patch hash" >&2; exit 1; }
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null 2>&1; then
  echo "The Chromium build is still running; wait before applying patch 032." >&2
  exit 1
fi

fingerprint_patch_dir="$fingerprint_path/patches/extra/fingerprint"
mkdir -p "$fingerprint_patch_dir"
cp "$patch_path" "$fingerprint_patch_dir/032-webgpu-template-identity.patch"
...
markers=(ApplyPrismWebGpuTemplateIdentity PrismWindowsWebGpuArchitecture blackwell)
applied=0
for marker in "${markers[@]}"; do
  grep -Fq "$marker" "$source_file" && applied=$((applied + 1))
done
if (( applied != 0 && applied != ${#markers[@]} )); then
  echo "The WebGPU template identity patch is only partially applied." >&2
  exit 1
fi
if (( applied == 0 )); then
...
lock["prismWebGpuTemplateIdentityPatchSha256"] = sys.argv[4]
lock["patches"] = [
...
echo "Prism WebGPU template identity patch is ready."
