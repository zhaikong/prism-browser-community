...
[[ -f "$patch_dir/031-windows-tts-runtime.patch" ]] || patch_dir="$script_dir/../kernel-patches"
patch_path="$patch_dir/031-windows-tts-runtime.patch"
...
markers=('StartPrismTtsWorkerThread' 'PrismSpEnumTokens' 'speech_synthesizer_->GetVoice' '[PrismTTS]' 'PrismTtsMtaWorker')

for path in "$patch_path" "$source_file" "$build_lock_path"; do
  [[ -f "$path" ]] || { echo "Required Windows TTS runtime patch input is missing: $path" >&2; exit 1; }
done
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null; then
  echo "Chromium is still compiling. Wait for Ninja before applying patch 031." >&2
  exit 1
fi
...
if actual != patch["sha256"]:
    raise SystemExit("Windows TTS runtime patch hash mismatch")
...
grep -Fq 'that as success hides every installed desktop SAPI voice' "$source_file" \
  || { echo "Patch 030 is missing. Apply the v16 patch before patch 031." >&2; exit 1; }
mkdir -p "$fingerprint_path/patches/extra/fingerprint"
cp "$patch_path" "$fingerprint_path/patches/extra/fingerprint/031-windows-tts-runtime.patch"
...
for marker in "${markers[@]}"; do
  grep -Fq "$marker" "$source_file" && applied_marker_count=$((applied_marker_count + 1))
done
if [[ "$applied_marker_count" -ne 0 && "$applied_marker_count" -ne "${#markers[@]}" ]]; then
  echo "The Windows TTS runtime patch is only partially applied. Restore a clean v16 source tree before continuing." >&2
  exit 1
fi
if [[ "$applied_marker_count" -eq 0 ]]; then
...
lock["prismWindowsTtsRuntimePatchSha256"] = next(
    item["sha256"] for item in contract["patches"]
    if item["file"] == "031-windows-tts-runtime.patch")
lock["patches"] = [
...
echo "Prism Windows TTS runtime patch is ready in the shared source tree."
