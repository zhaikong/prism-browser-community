...
[[ -f "$patch_dir/029-native-locale-surfaces-v4.patch" ]] || patch_dir="$script_dir/../kernel-patches"
patch_path="$patch_dir/029-native-locale-surfaces-v4.patch"
...
markers=(
  'third_party/blink/renderer/modules/speech/speech_synthesis.cc::render_identity == "v4"'
  'third_party/blink/renderer/platform/fonts/font_cache.cc::render_identity == "v4"'
  'third_party/blink/renderer/core/dom/document.cc::render_identity == "v3" || render_identity == "v4"'
  'third_party/blink/renderer/modules/webaudio/offline_audio_context.cc::render_identity != "v4"'
  'third_party/blink/renderer/modules/webgl/webgl_rendering_context_base.cc::coherent_render_identity'
  'third_party/blink/renderer/modules/canvas/canvas2d/base_rendering_context_2d.cc::render_identity != "v3" && render_identity != "v4"'
  'third_party/blink/renderer/core/html/canvas/text_metrics.cc::render_identity == "v3" || render_identity == "v4"'
  'third_party/blink/renderer/platform/graphics/static_bitmap_image.cc::render_identity == "v3" || render_identity == "v4"'
  'third_party/blink/renderer/modules/webaudio/offline_audio_destination_node.cc::render_identity == "v3" || render_identity == "v4"'
)

for path in "$patch_path" "$build_lock_path"; do
  [[ -f "$path" ]] || { echo "Required native locale surfaces v4 input is missing: $path" >&2; exit 1; }
done
for item in "${markers[@]}"; do
  relative="${item%%::*}"
  [[ -f "$source_path/$relative" ]] \
    || { echo "Prepared Chromium source is incomplete: $source_path/$relative" >&2; exit 1; }
done
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null; then
  echo "Chromium is still compiling. Wait for Ninja before applying v4." >&2
  exit 1
fi
...
if actual != patch["sha256"]:
    raise SystemExit("Native locale surfaces v4 patch hash mismatch")
...
grep -Fq "GetDocument().GetNoiseFactorX() != 1.0" \
  "$source_path/third_party/blink/renderer/core/dom/element.cc" \
  || { echo "Patch 028 is missing. Apply the v14 kernel patches before patch 029." >&2; exit 1; }
mkdir -p "$fingerprint_path/patches/extra/fingerprint"
cp "$patch_path" "$fingerprint_path/patches/extra/fingerprint/029-native-locale-surfaces-v4.patch"
...
for item in "${markers[@]}"; do
  relative="${item%%::*}"
  marker="${item#*::}"
  grep -Fq "$marker" "$source_path/$relative" && ((marker_count += 1)) || true
done
if (( marker_count != 0 && marker_count != ${#markers[@]} )); then
  echo "Native locale surfaces v4 is only partially applied." >&2
  exit 1
fi
...
lock["prismNativeLocaleSurfacesV4PatchSha256"] = next(
    item["sha256"] for item in contract["patches"]
    if item["file"] == "029-native-locale-surfaces-v4.patch")
lock["patches"] = [
...
echo "Prism native locale surfaces v4 patch is ready."
