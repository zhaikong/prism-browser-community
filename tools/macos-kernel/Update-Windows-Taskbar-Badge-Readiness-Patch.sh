...
patch_path="$script_dir/patches/044-windows-taskbar-badge-readiness.patch"
[[ -f "$patch_path" ]] || patch_path="$script_dir/../kernel-patches/044-windows-taskbar-badge-readiness.patch"

read -r patch_hash < <(python3 - "$lock_path" <<'PY'
import json, sys
lock = json.load(open(sys.argv[1]))
print(next(item["sha256"] for item in lock["patches"] if item["file"] == "044-windows-taskbar-badge-readiness.patch"))
PY
)
[[ "$(shasum -a 256 "$patch_path" | awk '{print $1}')" == "$patch_hash" ]] || { echo "Unexpected patch 044 hash" >&2; exit 1; }

window_property_source="$source_path/chrome/browser/ui/views/frame/browser_window_property_manager_win.cc"
[[ -f "$window_property_source" ]] || { echo "Prepared Chromium source is missing" >&2; exit 1; }
...
cp "$patch_path" "$fingerprint_patch_dir/044-windows-taskbar-badge-readiness.patch"
...
window_marker=0
grep -Fq 'SchedulePrismProfileOverlay' "$window_property_source" && window_marker=1
if (( window_marker == 1 )); then
  echo "Patch 044 is already applied."
else
...
echo "Prism Windows taskbar badge readiness patch is ready."
