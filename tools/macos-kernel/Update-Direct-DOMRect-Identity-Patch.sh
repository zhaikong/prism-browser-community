...
[[ -f "$patch_dir/027-direct-domrect-identity.patch" ]] || patch_dir="$script_dir/../kernel-patches"
patch_path="$patch_dir/027-direct-domrect-identity.patch"
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null; then
  echo "Chromium is still compiling. Wait for Ninja to finish before applying direct DOMRect identity." >&2
  exit 1
fi
...
if actual != patch["sha256"]:
    raise SystemExit("Direct DOMRect identity patch hash mismatch")
...
grep -Fq "kNoiseMask = 0xffffu" "$document_path" \
  || { echo "DOMRect seed mixing is missing. Apply patches 023 and 024 before patch 027." >&2; exit 1; }
mkdir -p "$fingerprint_path/patches/extra/fingerprint"
cp "$patch_path" "$fingerprint_path/patches/extra/fingerprint/027-direct-domrect-identity.patch"
...
if (( marker_count != 0 && marker_count != ${#markers[@]} )); then
  echo "Direct DOMRect identity is only partially applied." >&2
  exit 1
fi
...
lock["prismDirectDomRectIdentityPatchSha256"] = next(
    item["sha256"] for item in contract["patches"]
    if item["file"] == "027-direct-domrect-identity.patch")
lock["patches"] = [
...
echo "Prism direct DOMRect identity patch is ready."
