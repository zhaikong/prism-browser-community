...
[[ -f "$patch_dir/023-coherent-render-identity-v3.patch" ]] || patch_dir="$script_dir/../kernel-patches"
patch_path="$patch_dir/023-coherent-render-identity-v3.patch"
...
if pgrep -f "ninja.*$source_path/out/Default" >/dev/null; then
  echo "Chromium is still compiling. Wait for Ninja to finish before applying coherent render identity v3." >&2
  exit 1
fi
...
if actual != patch["sha256"]:
    raise SystemExit("Coherent render identity v3 patch hash mismatch")
...
cp "$patch_path" "$fingerprint_path/patches/extra/fingerprint/023-coherent-render-identity-v3.patch"
...
markers=(
  "$source_path/third_party/blink/renderer/core/dom/document.cc:PrismRenderIdentityNoise"
  "$source_path/third_party/blink/renderer/modules/canvas/canvas2d/base_rendering_context_2d.cc:PrismCanvasTextOffset"
  "$source_path/third_party/blink/renderer/modules/speech/speech_synthesis.cc:render_identity != \"v3\""
  "$source_path/third_party/blink/renderer/platform/fonts/font_cache.cc:render_identity != \"v3\""
  "$source_path/third_party/blink/renderer/modules/webaudio/offline_audio_context.cc:render_identity != \"v1\" && render_identity != \"v3\""
  "$source_path/third_party/blink/renderer/modules/webgl/webgl_rendering_context_base.cc:render_identity != \"v1\" && render_identity != \"v3\""
)
marker_count=0
...
if (( marker_count != 0 && marker_count != ${#markers[@]} )); then
  echo "Coherent render identity v3 is only partially applied." >&2
  exit 1
fi
...
lock["prismCoherentRenderIdentityPatchSha256"] = next(
    item["sha256"] for item in contract["patches"]
    if item["file"] == "023-coherent-render-identity-v3.patch")
lock["patches"] = [
...
echo "Prism coherent render identity v3 patch is ready."
